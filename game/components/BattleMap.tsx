...
function drawMinimap(c: HTMLCanvasElement | null, cam: Camera, opt: ViewOptions, w: World, r: Renderer, vw: number, vh: number, zoom: number, clashes: Clash[], now: number) {
    ...
    for (let i = 0; i < w.n; i += stride) {
        ...
        const color = (opt.ownerColors && w.owner[i] && opt.ownerColors[w.owner[i] as string]) || COLOR_HEX[opt.colors[s]];
    ...
    for (const [color, dots] of dotsByColor.entries()) {
        ...
        for (let k = 0; k < dots.length; k += 2) {
            g.rect(dots[k], dots[k + 1], dot, dot);
        }
        g.fill();
    }
    ...
    // ---- V2: Minimap Draw Depots (Kho lương)
    for (const d of w.depots) {
        if (d.claimed || d.expired) continue;
        const px = X(d.tx * 32 + 16), py = Y(d.ty * 32 + 16);
        if (px < -2 || py < -2 || px > S + 2 || py > S + 2) continue;
...
export default function BattleMap({
    ...
    // Lấy viewer (side) từ multiplayer hoặc mặc định là 0 (Tây) khi chơi AI
    ...
    const handleStartDeployment = useCallback((layout: { type: number, px: number, py: number, ownerId?: string | null }[]) => {
        ...
        const westOwners = new Set<string>();
        const eastOwners = new Set<string>();
        for (let i = 0; i < world.n; i++) {
            if (world.owner[i]) {
                if (world.side[i] === 0) westOwners.add(world.owner[i]!);
                else eastOwners.add(world.owner[i]!);
            }
        }
        const ownerColors: Record<string, string> = {};
        const westPalette = ["#64ff64", "#00ffff", "#4169e1", "#00fa9a"];
        const eastPalette = ["#ff3232", "#ff9632", "#b026ff", "#ff1493"];
        
        // Cố gắng để tướng chính (nếu có) lấy màu đầu tiên, còn lại sắp xếp theo ID để đồng bộ mọi client
        const sortOwners = (arr: string[]) => arr.sort((a, b) => {
            const isGeneralA = players?.find(p => p.playerId === a)?.isGeneral ? 1 : 0;
            const isGeneralB = players?.find(p => p.playerId === b)?.isGeneral ? 1 : 0;
            return isGeneralB - isGeneralA || a.localeCompare(b);
        });
        
        sortOwners(Array.from(westOwners)).forEach((id, i) => ownerColors[id] = westPalette[i % westPalette.length]);
        sortOwners(Array.from(eastOwners)).forEach((id, i) => ownerColors[id] = eastPalette[i % eastPalette.length]);

        optRef.current = { ...optRef.current, viewer: mySide, objKnown: knownRef.current, myId: myPlayerId, ownerColors };
                    ...
                    blocks.forEach((b: any) => {
                        ...
                        for (let c = 0; c < 10; c++) {
                            for (let r = 0; r < 10; r++) {
                                ...
                                layout.push({ type: b.type, px, py, ownerId: b.ownerId, side, squadId: currentSquad });
                            }
                        }
                    });
                };
            ...
            {showLeftUI && (
                <div className="ts-wood absolute left-2 top-2 w-[292px] text-[var(--cream)]">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-1">
                        ...
                        <button className="ts-btn red text-sm" disabled={!!loading || !mode} title="Toàn quân tấn công thẳng vào Thành địch (bỏ các cứ điểm!)" onClick={() => dispatchCommand({ action: "charge", sides: [PLAYER] })}>
                        ...
                        <button className="ts-btn text-sm" onClick={() => { if (confirm("Bạn có chắc muốn thoát ra Menu?")) onExit(); }}>
                            <img src={`${UI}/Icons/Icon_10.png`} alt="" className="h-6 w-6" /> Thoát trận
                        </button>
                        <button className="ts-btn text-sm red" onClick={() => { if (confirm("Bạn có chắc muốn ĐẦU HÀNG?")) worldRef.current?.surrender(PLAYER); }}>
                            <img src={`${UI}/Icons/Icon_24.png`} alt="" className="h-6 w-6" /> Đầu hàng
                        </button>
                ...
                <div className="ts-paper absolute bottom-2 left-1/2 flex -translate-x-1/2 items-center gap-3 text-[var(--ink)]">
                    ...
                    <div className="flex flex-col gap-1 border-l border-[#3b2416]/30 pl-3">
                        ...
                        <div className="grid grid-cols-2 gap-1">
                            ...
                            <button className="ts-btn !min-h-[36px] text-[11px]" onClick={() => { const w = worldRef.current; if (w) dispatchCommand({ action: "rally", sel: Array.from(w.selected()) }); }}>Quay đầu (G)</button>
                            <button className="ts-btn !min-h-[36px] text-[11px]" onClick={() => { const w = worldRef.current; if (w) dispatchCommand({ action: "hold", sel: Array.from(w.selected()) }); }}>Giữ (H)</button>
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div className="ts-paper mt-2 max-w-[520px] text-center text-[var(--ink)]">
                            ...
                            <div className="mt-2 flex justify-center gap-2">
                                <button className="ts-btn red text-sm" onClick={onExit}>Thoát ra Menu</button>
